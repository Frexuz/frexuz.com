export const CVItem = ({ name, style, dates, role, texts, tags }: any) => {
  return (
    <div
      ...
      style={style}
    >
      <div className="mb-5 w-full justify-start border-b border-pink-600 pb-2.5 md:mb-0 md:w-auto md:justify-end md:border-r md:pr-5">
          ...
          {tags &&
            tags.map((tag, index) => (
              <Tag
                ...
                text={tag}
                small
                className="m-0.5 bg-gray-300 text-gray-800"
              />
            ))}
        </div>
      </div>
